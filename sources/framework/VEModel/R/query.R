...
ve.query.export <- function(
  exporter="csv",  # One of the defined exporter types; see export.R or openExporter() with no arguments
  connection=NULL, # Used to instantiate exporter; see export.R and docs for specific exporter drivers
  partition=c(Scenario="merge",Group="merge",Table="name") # Default partition puts table in connection "root"
  ...
  if ( any(grepl("%(queryname|timestamp)%",ExtractFile)) ) {
    ExtractTable <- stringr::str_replace(ExtractFile,"%queryname%",ExtractName)
    ExtractTable <- stringr::str_replace(ExtractFile,"%timestamp%",format(Sys.time(),"%Y_%m_%d-%H_%M"))
  }
