# I was inspired by Kay Cichini recent post on creating a a tcl/tk
# dialog box for users to enter variable values. I am going to have a
# use for this very soon so took some time to make it a bit more
# generic. What I wanted is a function that takes a vector (of
# variable names) of arbitrary length, create a dialog box for an
# input for each, and return the values in a list. While I was at it
# I also provided an optional separate vector for the labels
# (defaults to the variable names) that the user will see and a
# vector of functions used to convert the text input into another
# data format (e.g. convert characters to numeric values). Obviously
# using built in functions works great but one could also easily
# exploit this feature to write custom data validation functions. The
# function is hosted on Gist and embedded bellow. Here are some very
# basic examples:
